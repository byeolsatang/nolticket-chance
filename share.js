(function(){
  ...
  function addStyle(){
    ...
    style.textContent = `
      ...
      .share-pass-note{margin:8px 0 0;color:var(--muted,#6b6b8a);font-size:11px;line-height:1.65;}
      .share-pass-note a{color:var(--accent3,#4af0c4);font-weight:800;text-decoration:none;}
  ...
  function buildShareCard(kind){
    ...
    card.id = isReadme ? 'readme-share-section' : 'app-share-section';
    card.innerHTML = '<div class="share-main"><div class="share-copy-block"><div class="share-kicker">Share</div><div class="share-title">'+title+'</div><p class="share-text">'+text+'</p></div><div class="share-actions"><a href="'+xUrl+'" target="_blank" rel="noopener noreferrer" aria-label="Xでシェア" title="Xでシェア">𝕏</a><a href="'+threadsUrl+'" target="_blank" rel="noopener noreferrer" aria-label="Threadsでシェア" title="Threadsでシェア">＠</a><button type="button" class="share-copy" aria-label="リンクをコピー" title="リンクをコピー">⛓</button></div></div><p class="share-pass-note">合言葉は定期的に更新し、Instagramでお知らせします。<a href="'+INSTAGRAM_URL+'" target="_blank" rel="noopener noreferrer">Follow me!</a></p><div class="share-copy-message" aria-live="polite"></div>';
  ...
  function insertShareSections(){
    ...
    var oldProfileShare = document.getElementById('profile-share-section');
    if (oldProfileShare) oldProfileShare.remove();
